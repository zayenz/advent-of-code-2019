...
impl OpArgs2 {
    fn double_from(store: &[i32], pc: usize) -> OpArgs2 {
        OpArgs2 {
            in1: store[pc],
            in2: store[pc + 1],
        }
...
impl Op {
    fn decode(store: &[i32], pc: usize) -> Result<Op, Error> {
        ...
        match opcode {
            ...
            5 => Ok(Op::JumpIfTrue(
                OpArgs2::double_from(store, pc + 1),
                arg_modes,
            )),
            6 => Ok(Op::JumpIfFalse(
                OpArgs2::double_from(store, pc + 1),
            ...
            7 => Ok(Op::LessThan(OpArgs3::triple_from(store, pc + 1), arg_modes)),
            8 => Ok(Op::Equal(OpArgs3::triple_from(store, pc + 1), arg_modes)),
            99 => Ok(Op::Halt),
    ...
    fn run(&self, store: &mut [i32], pc: usize, input: &mut Option<i32>) -> Status {
        match *self {
            ...
            Op::JumpIfTrue(OpArgs2 { in1, in2 }, (am1, am2, _)) => {
                let in1 = am1.get(store, in1);
                let in2 = am2.get(store, in2);
                if in1 != 0 {
                    Status::Continue(in2 as usize)
                } else {
                    Status::Continue(pc + 3)
                }
            }
            Op::JumpIfFalse(OpArgs2 { in1, in2 }, (am1, am2, _)) => {
                let in1 = am1.get(store, in1);
                let in2 = am2.get(store, in2);
                if in1 == 0 {
                    Status::Continue(in2 as usize)
            ...
            Op::LessThan(OpArgs3 { in1, in2, out }, (am1, am2, _am3)) => {
                let in1 = am1.get(store, in1);
                let in2 = am2.get(store, in2);
            ...
            Op::Equal(OpArgs3 { in1, in2, out }, (am1, am2, _am3)) => {
                let in1 = am1.get(store, in1);
                let in2 = am2.get(store, in2);
                let result = if in1 == in2 { 1 } else { 0 };
                store[out as usize] = result;
